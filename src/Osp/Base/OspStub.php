...
namespace VipClient\Osp\Base;

//use Thrift\Transport\TFramedTransport;
//use Thrift\Type\TMessageType;
//use Thrift\Transport\TSocket;
//use Thrift\Transport\TBufferedTransport;
use VipClient\Osp\Context\InvocationContextFactory;
...
//		\VipClient\Osp\Log\Logger::info("[OspStub][InitInvocation][Service-Version-Method][".$this->serviceName."-".$this->version."-".$method."]");
	...
	public function createRequestSign($accessToken, $appKey, $format, $language, $method, $service
		...
		return \VipClient\Osp\Util\HmacUtil::hmac($sign, $appSecret);
	}
	...
	/**
	 * 从环境变量提取URL信息
	 * @throws \VipClient\Osp\Exception\OspException
	 * @return multitype:number string
	 */
	public function getCallURLInfo($restAddr){
		if(!$restAddr) throw new \VipClient\Osp\Exception\OspException("The openapi appURL is null, please set the appURL value");
	...
	public function send_base($args) {
		$ctx = InvocationContextFactory::getInstance ();
		$oprot = new \VipClient\Osp\Protocol\JSONProtocol(new \VipClient\Osp\Buffer\MemoryBuffer());
		$args->write ( $oprot );
    ...
    $scheme = isset($url["scheme"]) ? $url["scheme"] : null;
		...
		$client = new \VipClient\Osp\Http\HttpClient($scheme, $host, $port);
		$request = array();
		
		$appKey = $ctx->getAppKey();
		if($appKey == null) throw new \VipClient\Osp\Exception\OspException("The openapi appKey is null, please set the appKey value");
		
		$appSecret = $ctx->getAppSecret();
		if($appSecret == null) throw new \VipClient\Osp\Exception\OspException("The openapi appSecret is null, please set the appSecret value");
		...
		$timestamp = round(\VipClient\Osp\Util\TimeUtil::currentTimeMillis()/1000);
		...
		try {
			...
			throw new \VipClient\Osp\Exception\OspException(\VipClient\Osp\Util\MessageUtil::getInvocationMsg($ctx, $e->getMessage()) , $e);
		}
	...
	public function receive_base($result) {
		...
		if(\VipClient\Osp\Base\OspStub::$RETURN_NULL != $response){
			$iprot = new \VipClient\Osp\Protocol\JSONProtocol(new \VipClient\Osp\Buffer\MemoryBuffer());
			...
			if(!"0" == $returnCode){
				...
				throw new \VipClient\Osp\Exception\OspException($returnMessage, $returnCode);
			}
